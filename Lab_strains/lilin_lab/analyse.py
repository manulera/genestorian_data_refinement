...
import pandas as pd
read_file  = pd.read_excel('DY-export-2.xlsx' )
read_file.to_csv('DY-export-2.tsv', sep='\t') 
...
allele_names = set([])
data = pandas.read_csv('DY-export-2.tsv', sep='\t', usecols = ['Genotype'])
...
import os 
import sys
from genestorian_data_refinement.pattern_identify import find
found = find('yfh1c', 'DY-export-2.tsv')
print(found)
...
def extract_pattern(pattern, input_file):
    pattern = 
