...
import pandas as pd
read_file  = pd.read_excel('Manu_Strains.xlsx' )
read_file.to_csv('Manu_Strains.tsv', sep='\t') 
...
allele_names = set([])
data = pandas.read_csv('Manu_strains.tsv', sep='\t', usecols = ['Genotype'])
...
allele_names = set([])
str = 'ajkfdjhf sdhib dihkfjn ilfehufe'
for a in re.split("\s",str):
        allele_names.add(a)
print(allele_names)
...
print(alleles_with_replaced_name)


# In[8]:


markers = ['terminator' ,'kanr','kanmx6','kanmx4','kanmx','hygr','hyg','hphmx','hphr','hph','natmx','natr','nat','kan','natmx6',r'\d*myc',r'\d*flag\d*']
...
tags = ['tdtomato','megfp','egfp','sfgfp','mcherry','cfp','spmneongreen','mneongreen','2xyfp','myfp','yfp', 'dcas9', 'grna', 'terminator', 'mcherry', 'mxi', 'mmaple3', 'meos3.2', 'gfp', 'ostir1(f74a)', 'bsdmx', 'mch', 'mtagbfp', 'z3ev', 'irfp', 'his5+']
...
counted = count_most_common_consecutive_N('pattern_identification.txt',15)
...
def find(text, file):
    ...
    with open(file,'rt') as ins:
        data = pandas.read_csv(ins, sep='\t', usecols = ['Genotype'])
        data['Genotype'] = data['Genotype'].astype(str)
        for genotype in data.Genotype:
            if text in genotype: # if the username shall be on column 3 (-> index 2)
