...
def is_gene(allele, gene_dict, genotype, replace_word):
    genes = []
    for allele in genotype:
        for name in re.findall(r'[a-z]{3}\d+', allele):
            if name.lower() in gene_dict.keys():
                gene = gene.replace(name.lower(), replace_word)
        genes.append(genes)
    return genes
...
def replace_allele_features(toml_file, genotype, replace_word):
    ...
    if feature_to_replace == 'gene':
        genedict = gene_dict(toml_file)
        replaced_word = is_gene(genedict, genotype, replace_word)
