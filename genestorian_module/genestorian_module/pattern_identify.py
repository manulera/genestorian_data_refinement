...
class Analyse():
    ...
    def replace_entity(self, alleles_with_replaced_name, identifiers, IDENTIFIER):
        for i in range(len(alleles_with_replaced_name)):
            for identifier in identifiers:
                alleles_with_replaced_name[i] = re.sub(
                    identifier, IDENTIFIER, alleles_with_replaced_name[i])
        return alleles_with_replaced_name

    def load_identifiers(self):
        f = toml.load('identifiers\identifiers.toml')
        data = toml.load(f)
        tags = []
        promoters = []
        markers = []
        for identifier in data['tag'].keys():
            tags.append(identifier)
        for identifier in data['marker'].keys():
            markers.append(identifier)
        for identifier in data['promoter'].keys():
    ...
    def replace_allele_names(self):
                    ...
                    if systematic_id in allele_dictionary:
                        for published_allele in allele_dictionary[systematic_id]:
                            if published_allele.lower() in genotype_allele:
                                genotype_allele = genotype_allele.replace(
        ...
        alleles_with_replaced_name = self.replace_entity(
            alleles_with_replaced_name, markers, 'MARKER')
        alleles_with_replaced_name = self.replace_entity(
            alleles_with_replaced_name, tags, 'TAG')
        alleles_with_replaced_name = self.replace_entity(
            alleles_with_replaced_name, promoters, 'PROMOTER')

        # replace the white spcaes with a single dash, a leading or trailing symbol
        for i in range(len(alleles_with_replaced_name)):
            alleles_with_replaced_name[i] = re.sub(
                r'[:-<\.]+', '-', alleles_with_replaced_name[i])
            alleles_with_replaced_name[i] = re.sub(
                r'^-', '', alleles_with_replaced_name[i])
            alleles_with_replaced_name[i] = re.sub(
                r'-$', '', alleles_with_replaced_name[i])
